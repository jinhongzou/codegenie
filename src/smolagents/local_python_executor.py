...
class LocalPythonExecutor(PythonExecutor):
    ...
    def get_local_executor_state(self, values_name='user'):
        """
        Retrieve specific parts of the executor's state.
        """
        exclude_keys = ["_print_outputs", "_operations_count"]
        if values_name == "user":
            return {k: v for k, v in self.state.items() if k not in exclude_keys}
        return self.state.get(values_name)
